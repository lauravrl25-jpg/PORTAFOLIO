...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 3. MENÚ RESALTA LA SECCIÓN ACTUAL (CORREGIDO: incluye main[id])
  const secciones = document.querySelectorAll("section[id], main[id]");
  const enlacesNav = document.querySelectorAll(".nav-link");
  ...
  // 4. APARICIÓN AL HACER SCROLL (CORREGIDO: incluye main)
  const objetivosReveal = document.querySelectorAll(
    "section, main, .servicio-card, .card, .proceso-step"
  );
